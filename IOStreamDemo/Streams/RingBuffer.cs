...
namespace IOStreamDemo.Streams
{
    ...
    public class BufferedStream
    {
        ...
        public RingBuffer Rb { get; set; } = new RingBuffer(1048576);
        byte[] rxArray = new byte[1048576];
        int readBytes = 0;
        CancellationTokenSource cts = new();

        public BufferedStream(IDriverCommStream stream)
        {
            Stream = stream;
        }
        ...
        public void RecvService()
        {
            try
            {
                while (!cts.IsCancellationRequested)
                {
                    ...
                    {
                        readBytes = Stream.Read(rxArray, 0, 1048576);
                        Rb.Write(rxArray, 0, readBytes);
                    }
                }
            }
            catch (Exception ex)
