...
namespace IOStreamDemo.Streams
{
    ...
    public class BufferedStream : IDriverCommStream
    {
        ...
        public BufferedStream(IDriverCommStream stream, int bufferSize = 1048576)
        {
            Stream = stream;
            BufferSize = bufferSize;
            Rb = new RingBuffer(bufferSize);
            RxArray = new byte[BufferSize];
        }
        ...
        public void RecvService()
        {
            try
            {
                ...
                {
                    if (Stream.ReadAvailable)
                    {
                        int bytesRead = Stream.Read(RxArray, 0, 1048576);
...
int IDriverCommStream.Read(byte[] buffer, int offset, int count)
        {
    throw new NotImplementedException();
}
    }
